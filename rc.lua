...
if awesome.startup_errors then
  naughty.notify({
    ...
    title = "Ептель! Ошибка во время старта, проверь все внимательно.",
...
do
  ...
  awesome.connect_signal("debug::error",
    function (err)
      ...
      naughty.notify({
        preset = naughty.config.presets.critical,
        title = "Ай-яй, произошла ошибочка.",
        text = err
      })
      in_error = false
    end)
...
-->>[[ Настройка панелей ]]
local mouse = { LEFT = 1, MIDDLE = 2, RIGHT = 3, WHEEL_UP = 4, WHEEL_DOWN = 5 } --Кнопки мыши
...
--Часы и календарь
date = awful.widget.textclock("%a %d %b, %H:%M")
...
globalkeys = awful.util.table.join(
  ...
  awful.key({ modkey,           }, "w", function () start_menu:show() end),
  -- Layout manipulation
  ...
  awful.key({ modkey }, "x",
    function ()
      ...
      promp_tbox[mouse.screen].widget,
      awful.util.eval, nil,
